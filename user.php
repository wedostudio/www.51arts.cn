...
elseif ($action == 'avatar')
{
    ...
    if ($source_pic)
    {
        ...
        else
        {
            ...
            //保存路径
            $savePath = "$dir/source_$filename.".$sourceExtendName;
            //当前头像基于原图的初始化参数（只有上传原图时才会发送该数据，且发送的方式为POST），用于修改头像时保证界面的视图跟保存头像时一致，提升用户体验度。
            //修改头像时设置默认加载的原图url为当前原图url+该参数即可，可直接附加到原图url中储存，不影响图片呈现。
            $init_params = $_POST["__initParams"];
            $result['sourceUrl'] = toVirtualPath($_SERVER['DOCUMENT_ROOT'].$savePath).$init_params;
            move_uploaded_file($source_pic["tmp_name"], $_SERVER['DOCUMENT_ROOT'].$savePath);
            $success_num++;
        }
    }
    //<------------------------------------------------------------------------处理原始图片结束
    ...
    for ( $i = 0; $i < $avatars_length; $i++ )
    {
        ...
        {
            $savePath = "$dir/avatar" . $avatar_number . "_$filename.jpg";
            $result['avatarUrls'][$i] = toVirtualPath($_SERVER['DOCUMENT_ROOT'].$savePath);
            move_uploaded_file($avatar["tmp_name"], $_SERVER['DOCUMENT_ROOT'].$savePath);
            $sql = "UPDATE ".$ecs->table('users') ." SET avatar='$savePath' WHERE user_id=$user_id";
            $db->query($sql);
            $success_num++;
        }
    }
